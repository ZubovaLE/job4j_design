...
Если данные методы не переопределяются, то используются hashCode и equals по умолчанию, реализуемые в классе Object.

Метод equals() по умолчанию в Object:
...
Он принимает в качестве аргумента ссылочную переменную и проверяет, ссылается ли они на тот же объект (ту же область
памяти, если быть точнее), что и объект, к которому мы применили метод .equals(). Но рассмотрим ситуацию с
пользовательским классом Person:
...
Допустим, мы создали два объекта Person personOne и Person personTwo с одинаковыми полями. Тогда такие объекты будут
формально разными по equals(), так как они ссылаются на разные участки памяти, хотя по смыслу они одинаковы. Поэтому в
собственных классах необходимо переопределять методы hashCode и equals.
...
+ TreeSet - структура данных в виде дерева, в котором все объекты хранятся в отсортированном по возрастанию порядке.
  TreeSet инкапсулирует в себе TreeMap, который в свою очередь использует сбалансированное бинарное красно-черное дерево
  для хранения элементов. Можно управлять порядком при помощи компаратора.
